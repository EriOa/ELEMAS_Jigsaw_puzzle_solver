...
def Load_images(num, invert=False):
    imagearray = []
    imagearray_gray = []
    for i in range(num):
        path = "C:/Users/Erik/OneDrive - Universitetet i Stavanger/Erik Skole (Master)/ELEMAS/Bilder/Cropped_ImageCroppedImage{}.jpg".format(i)
        img = cv2.imread(path)
        if invert:
            img = cv2.flip(img,1)
...
def saveMatchedPairFigure(matches, pieceSides_col):
    ...
    for i, match in enumerate(matches):
        ...
        # BGR to RGB
        #pieceSides_col_RGB1 = cv2.cvtColor(pieceSides_col[hole_index][hole_side], cv2.COLOR_BGR2RGB)
        #pieceSides_col_RGB2 = cv2.cvtColor(pieceSides_col[head_index][head_side], cv2.COLOR_BGR2RGB)
